...
func vmiContainsNonMigratablePCIHostDevices(vmi *v1.VirtualMachineInstance) bool {
	usesSingleNVIDIAGPU :=  len(vmi.Spec.Domain.Devices.GPUs) == 1 && strings.HasPrefix(vmi.Spec.Domain.Devices.GPUs[0].DeviceName, "nvidia.com")

	return len(vmi.Spec.Domain.Devices.HostDevices) > 0 || len(vmi.Spec.Domain.Devices.GPUs) > 1 || !usesSingleNVIDIAGPU
}
