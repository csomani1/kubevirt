...
var _ = Describe("Premigration Hook Server", func() {
	Context("vGPU Dedicated Hook", func() {
		It("should update vGPU mdev uuid according to target node config", func() {
			...
			vmi.Annotations[v1.TargetMdevUUIDAnnotation] = targetUUID

			Expect(VGPULiveMigration(vmi, &domain)).NotTo(HaveOccurred(), "failed to modify domain")
		...
		It("should fail if there is more than 1 vGPU", func() {
			...
			Expect(VGPULiveMigration(vmi, &domain)).To(MatchError("the migrating vmi should only have one vGPU"))
		})

		It("should fail if GPU is not an mdev vGPU", func() {
			By("creating a VMI and libvirt domain with 1 PCI GPU")
			targetUUID := "3ad93aea-4e81-49a9-b1af-9d020373a357"
			...
			Expect(VGPULiveMigration(vmi, &domain)).To(MatchError("failed to retrieve mdev vGPU from domain"))
		})
	})
})
